        ...
        if self._report_mem_usage:
            ...
            try:
                with tf.compat.v1.name_scope(self.id + '_mem'), tf.device(device.name), tf.control_dependencies([loss]):
                    deps.append(autosummary.autosummary(self.id + "/mem_usage_gb", tf.raw_ops.BytesInUse() / 2**30))
    ...
    def apply_updates(self, allow_no_op: bool = False) -> tf.Operation:
        ...
        for device_idx, device in enumerate(self._devices.values()):
            with tfutil.absolute_name_scope(self.scope + "/Apply%d" % device_idx), tf.device(device.name):
                ...
                all_ok = tf.reduce_all(tf.stack([acc_ok] + [tf.reduce_all(tf.math.is_finite(g)) for g in device.grad_acc.values()]))
