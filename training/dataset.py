...
class TFRecordDataset:
    def __init__(self,
        ...
        with tf.compat.v1.name_scope('Dataset'), tf.device('/cpu:0'), tf.control_dependencies(None):
            ...
            for tfr_file, tfr_shape, tfr_lod in zip(tfr_files, tfr_shapes, tfr_lods):
                ...
                dset = tf.compat.v1.data.Dataset.zip((dset, self._tf_labels_dataset))
    ...
    def get_minibatch_tf(self):
        ...
        if self.mirror_augment:
            ...
            images = tf.compat.v1.where(tf.random.uniform([tf.shape(images)[0]]) < 0.5, images, tf.reverse(images, [3]))
