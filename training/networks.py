...
def apply_bias_act(x, act='linear', gain=None, lrmul=1, clamp=None, bias_var='bias', trainable=True):
    b = tf.compat.v1.get_variable(bias_var, shape=[x.shape[1]], initializer=tf.compat.v1.initializers.zeros(), trainable=trainable) * lrmul
...
def conv2d(x, w, up=False, down=False, resample_kernel=None, padding=0):
    ...
    if up:
        ...
        x = tf.nn.conv2d(input=x, filters=w, data_format='NCHW', strides=[1,1,1,1], padding=padding_mode)
    return x
...
def modulated_conv2d_layer(x, y, fmaps, kernel, up=False, down=False, demodulate=True, resample_kernel=None, lrmul=1, fused_modconv=False, trainable=True, use_spectral_norm=False):
    ...
    if x.dtype.name == 'float16' and not fused_modconv and demodulate:
        w *= np.sqrt(1 / np.prod(wshape[:-1])) / tf.reduce_max(input_tensor=tf.abs(w), axis=[0,1,2]) # Pre-normalize to avoid float16 overflow.
    ...
    if x.dtype.name == 'float16' and not fused_modconv and demodulate:
        s *= 1 / tf.reduce_max(input_tensor=tf.abs(s)) # Pre-normalize to avoid float16 overflow.
    ww *= tf.cast(s[:, np.newaxis, np.newaxis, :, np.newaxis], w.dtype) # [BkkIO] Scale input feature maps.

    # Demodulate.
    if demodulate:
        d = tf.math.rsqrt(tf.reduce_sum(input_tensor=tf.square(ww), axis=[1,2,3]) + 1e-8) # [BO] Scaling factor.
        ww *= d[:, np.newaxis, np.newaxis, np.newaxis, :] # [BkkIO] Scale output feature maps.

    # Reshape/scale input.
    if fused_modconv:
        x = tf.reshape(x, [1, -1, x.shape[2], x.shape[3]]) # Fused => reshape minibatch to convolution groups.
        w = tf.reshape(tf.transpose(a=ww, perm=[1, 2, 3, 0, 4]), [ww.shape[1], ww.shape[2], ww.shape[3], -1])
    else:
...
def normalize_2nd_moment(x, axis=1, eps=1e-8):
    return x * tf.math.rsqrt(tf.reduce_mean(input_tensor=tf.square(x), axis=axis, keepdims=True) + eps)
...
def minibatch_stddev_layer(x, group_size=None, num_new_features=1):
    ...
    y -= tf.reduce_mean(input_tensor=y, axis=0)            # [GnFcHW] Subtract mean over group.
    y = tf.reduce_mean(input_tensor=tf.square(y), axis=0)  # [nFcHW]  Calc variance over group.
    y = tf.sqrt(y + 1e-8)                     # [nFcHW]  Calc stddev over group.
    y = tf.reduce_mean(input_tensor=y, axis=[2,3,4])       # [nF]     Take average over channels and pixels.
...
def apply_spectral_norm(w, state_var='sn', iterations=1, eps=1e-8):
    ...
    u_var = tf.compat.v1.get_variable(state_var, shape=[1,fmaps], initializer=tf.compat.v1.initializers.random_normal(), trainable=False)

    u = u_var
    for _ in range(iterations):
        v = tf.matmul(u, w_mat, transpose_b=True)
        v *= tf.math.rsqrt(tf.reduce_sum(input_tensor=tf.square(v)) + eps)
        u = tf.matmul(v, w_mat)
        sigma_inv = tf.math.rsqrt(tf.reduce_sum(input_tensor=tf.square(u)) + eps)
        u *= sigma_inv

    with tf.control_dependencies([tf.compat.v1.assign(u_var, u)]):
        return w * sigma_inv
...
def G_main(
    ...
    **kwargs,                                       # Arguments for sub-networks (mapping and synthesis).
    ...
    dlatent_avg = tf.compat.v1.get_variable('dlatent_avg', shape=[dlatent_size], initializer=tf.compat.v1.initializers.zeros(), trainable=False)
    if dlatent_avg_beta is not None:
        with tf.compat.v1.variable_scope('DlatentAvg'):
            batch_avg = tf.reduce_mean(input_tensor=dlatents[:, 0], axis=0)
    ...
    if style_mixing_prob is not None:
        with tf.compat.v1.variable_scope('StyleMix'):
            latents2 = tf.random.normal(tf.shape(input=latents_in))
            dlatents2 = components.mapping.get_output_for(latents2, labels_in, is_training=is_training, **kwargs)
            dlatents2 = tf.cast(dlatents2, tf.float32)
            layer_idx = np.arange(num_layers)[np.newaxis, :, np.newaxis]
            mixing_cutoff = tf.cond(
                pred=tf.random.uniform([], 0.0, 1.0) < style_mixing_prob,
                true_fn=lambda: tf.random.uniform([], 1, num_layers, dtype=tf.int32),
                false_fn=lambda: num_layers)
            dlatents = tf.compat.v1.where(tf.broadcast_to(layer_idx < mixing_cutoff, tf.shape(input=dlatents)), dlatents, dlatents2)
    ...
    if truncation_psi is not None:
        with tf.compat.v1.variable_scope('Truncation'):
            ...
            if truncation_cutoff is None:
                ...
                layer_psi = tf.compat.v1.where(layer_idx < truncation_cutoff, layer_psi * truncation_psi, layer_psi)
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    if use_noise:
        for layer_idx in range(num_layers - 1):
            ...
            noise_inputs.append(tf.compat.v1.get_variable(f'noise{layer_idx}', shape=shape, initializer=tf.compat.v1.initializers.random_normal(), trainable=False))
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        ...
        if use_noise:
            if randomize_noise:
                noise = tf.random.normal([tf.shape(input=x)[0], 1, x.shape[2], x.shape[3]], dtype=x.dtype)
            else:
                noise = tf.cast(noise_inputs[layer_idx], x.dtype)
            noise_strength = tf.compat.v1.get_variable('noise_strength', shape=[], initializer=tf.compat.v1.initializers.zeros())
    ...
    with tf.compat.v1.variable_scope('4x4'):
        with tf.compat.v1.variable_scope('Const'):
            ...
            x = tf.compat.v1.get_variable('const', shape=[1, fmaps, 4, 4], initializer=tf.compat.v1.initializers.random_normal())
            x = tf.tile(tf.cast(x, dtype), [tf.shape(input=dlatents_in)[0], 1, 1, 1])
    ...
    **_kwargs,                          # Ignore unrecognized keyword args.
    ...
    def adrop(x):
        if adaptive_dropout != 0:
            s = [tf.shape(input=x)[0], x.shape[1]] + [1] * (x.shape.rank - 2)
    ...
    if use_pagan:
        with tf.compat.v1.variable_scope('PAGAN'):
            ...
            rnd = tf.random.uniform([tf.shape(input=images_in)[0], pagan_num])
            pagan_bits = tf.cast(rnd < prob, dtype=tf.float32)
            pagan_signs = tf.reduce_prod(input_tensor=1 - pagan_bits * 2, axis=1, keepdims=True)
    ...
    with tf.compat.v1.variable_scope('Output'):
        if label_size > 0:
            ...
            x = tf.reduce_sum(input_tensor=x * labels_in, axis=1, keepdims=True) / np.sqrt(mapping_fmaps)
        else:
